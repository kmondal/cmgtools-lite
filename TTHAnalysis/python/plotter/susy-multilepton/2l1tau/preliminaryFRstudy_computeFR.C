...
void doRatioInt(TH1* h1, TH1* h2, TString sample)
{
  ...
  h1->Draw();
  h2->Draw("same");
  c->Print(TString("met_")+sample+TString(".png"));
  c->Print(TString("met_")+sample+TString(".pdf"));
  c->Clear(); 
  ...
  c->Print(TString("ratio_")+sample+TString(".png"));
  c->Print(TString("ratio_")+sample+TString(".pdf"));
